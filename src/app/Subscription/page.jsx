...
export default function SubscriptionPage() {
  ...
  // Prime with cached subscription for instant real-data render
  useEffect(() => {
    try {
  ...
  const fetchSubscription = async () => {
    try {
      ...
      setSubscription(sub)
      // cache for instant loads on revisit/navigation
  ...
  const getAvailablePlans = () => [
    {
      ...
      features: [
        ...
        "✔ FastTag Recharge Included",
        "✔ GPS Device (₹1600 additional cost)",
      ],
            ...
            {getAvailablePlans().map((plan) => {
              ...
              return (
                <div key={plan.id} className="border rounded-lg shadow-sm border-gray-200 p-6 bg-white relative overflow-hidden">
                  ...
                  {plan.popular && !active && (
                    <div className="bg-yellow-200 text-center rounded-md py-1 font-semibold text-black mb-4">
                      MOST POPULAR
                    </div>
                  )}
