                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className={`text-sm ${!notification.isRead ? 'text-gray-700 font-medium' : 'text-gray-500'}`}>
...
const AdminDashboard = () => {
  ...
  // Process notifications from subadmin data
  const processNotifications = (subAdminData) => {
    if (subAdminData?.notifications && Array.isArray(subAdminData.notifications)) {
      // Add isRead property to each notification (default to false for new notifications)
      const processedNotifications = subAdminData.notifications.map(notification => ({
        ...notification,
        isRead: false // You can modify this logic based on your read status tracking
      }))
  ...
  // Fetch notifications from subadmin data
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${baseURL}api/admin/getSubAdmin/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })

      if (response.data.success) {
        processNotifications(response.data.subAdmin)
      }
    } catch (error) {
  ...
  const markNotificationAsRead = async (notificationId) => {
    try {
      ...
      // Optional: Make API call to mark as read on backend if you have this endpoint
      // const token = localStorage.getItem("token")
  ...
  const markAllNotificationsAsRead = async () => {
    try {
      ...
      // Optional: Make API call to mark all as read on backend if you have this endpoint
      // const token = localStorage.getItem("token")
  ...
  useEffect(() => {
    const checkUserStatusAndFetchData = async () => {
      ...
      try {
        ...
        // Process notifications from the subadmin data
        processNotifications(responseData.subAdmin)
